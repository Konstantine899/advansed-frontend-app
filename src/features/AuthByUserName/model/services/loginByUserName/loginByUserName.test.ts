...
describe("loginByUserName.test", () => {
  ...
  test('success login', async () => {
    ...
    const thunk = new TestAsyncThunk(loginByUserName);// аргументом передаю async thunk
     const result = await thunk.callThunk({ username: "Константин", password: "1" }); // передаю arg

    expect(thunk.dispatch).toHaveBeenCalledWith(userActions.setAuthData(userValue)); // проверяю вызов dispatch с аргументом action
